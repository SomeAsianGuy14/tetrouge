class_name ShieldBar
extends Control

const BAR_WIDTH := 12
const SHIELD_COLOR := Color(0.78, 0.80, 0.84)
const BG_COLOR := Color(0.08, 0.08, 0.08, 0.8)
const GAP := 2.0
const MAX_BLOCKS := 8

var _charges: int = 0

func _ready() -> void:
	custom_minimum_size = Vector2(BAR_WIDTH, TetrisBoard.VISIBLE_ROWS * TetrisBoard.CELL_SIZE)

func update_charges(charges: int) -> void:
	_charges = charges
	queue_redraw()

func _draw() -> void:
	var bar_height := float(TetrisBoard.VISIBLE_ROWS * TetrisBoard.CELL_SIZE)
	var pixels_per_block := bar_height / float(MAX_BLOCKS)

	draw_rect(Rect2(0.0, 0.0, BAR_WIDTH, bar_height), BG_COLOR)

	var blocks := mini(_charges, MAX_BLOCKS)
	for i in range(blocks):
		var block_h := pixels_per_block - GAP
		var y := bar_height - (i + 1) * pixels_per_block
		draw_rect(Rect2(0.0, y, BAR_WIDTH, block_h), SHIELD_COLOR)

	if _charges > MAX_BLOCKS:
		var overflow_text := "+%d" % (_charges - MAX_BLOCKS)
		draw_string(ThemeDB.fallback_font, Vector2(0.0, bar_height - MAX_BLOCKS * pixels_per_block - 4.0), overflow_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 10, SHIELD_COLOR)

...
**Reason:** The event-type targeting model is replaced by `TechniqueEvaluator` dispatching on `effect_type` and reading `AttackContext`. Technique data no longer stores explicit event-type keys.
**Migration:** Techniques now declare an `effect_type` string and a `params` dictionary. The evaluator handles dispatch internally.

### Requirement: Technique pool for launch
**Reason:** All 10 original techniques are replaced by the 53-technique pool defined in the `technique-pool` capability spec.
**Migration:** Delete all 10 legacy `.tres` files. The new pool is defined in `openspec/specs/technique-pool/spec.md`.
